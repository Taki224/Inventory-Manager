...
def open(mainframe, userdetails):
    ...
    def remove_from_stock():
        ...
        params = {'auth_user': userdetails['username'], 'auth_password': userdetails['password'],
                  'change': int(quantity_entry.get()) * -1}
        r = requests.put(url=url, params=params)
        if r.status_code == 200:
            if json.loads(r.text)['success']:
                messagebox.showinfo("Success", "Product removed from stock!")
                selected_id = None
    ...
    # Add to Stock Button
    remove_button = ttk.Button(root, text="Remove from Stock", command=remove_from_stock)
